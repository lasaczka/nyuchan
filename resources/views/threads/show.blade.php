        ...
        <div class="card thread-form-card" id="post-form">
            ...
            <details class="reply-details thread-reply-details" @if($errors->any() || $quotePostId) open @endif>
                ...
                <form method="POST" action="{{ route('posts.store', ['board' => $board->slug, 'thread' => $thread->id]) }}" class="stack" enctype="multipart/form-data">
                    ...
                    <div>
                        <label for="reply-images">{{ __('ui.image_optional') }}</label>
                        <div class="file-picker">
                            <input id="reply-images" class="file-input-native" name="images[]" type="file" accept="image/jpeg,image/png,image/gif,image/webp" multiple>
                            <label for="reply-images" class="button secondary">{{ __('ui.choose_files') }}</label>
